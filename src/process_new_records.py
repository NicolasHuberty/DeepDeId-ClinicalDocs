...
from utils import CustomDataset,TextDataset,load_records_manual_process,store_eval_records, load_config_field,save_config_field,should_allocate_to_evaluation,load_records_eval_set
...
def process_new_records(project_name,new_predictions=100):
    ...
    train_model(project_name,train_records,train_labels,train_records_end)
    
    new_records,_,_,_ = load_records_manual_process(project_name,0)
    new_records = new_records[:new_predictions]
    make_prediction(project_name,new_records,train_records_end + 1)
    
    eval_records, eval_manual_labels,_,_ = load_records_eval_set(project_name,1,0,train_records_end)
    if(len(eval_records)>1):
        evaluate_model(project_name,eval_records,eval_manual_labels)
